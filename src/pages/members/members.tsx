...
const Members = () => {
    ...
    const groupedMembers = useMemo(() => {
        const groups = {
            회장단: MEMBERS.filter(
                (member) =>
                    (member.position === POSITION.회장 ||
                        member.position === POSITION.부회장) &&
                    member.year === selectedYear
            ),
            파트장: MEMBERS.filter(
                (member) =>
                    member.position === POSITION.파트장 &&
                    member.year === selectedYear
            ),
            챌린저: MEMBERS.filter(
                (member) =>
                    member.position === POSITION.챌린저 &&
                    member.year === selectedYear
            ).reduce((acc, member) => {
        ...
        <S.Container>
            ...
            {groupedMembers.회장단.length > 0 && (
                <>
                    ...
                    <S.Members>
                        {groupedMembers.회장단.map((member: TeamMember) => (
            ...
            {groupedMembers.파트장.length > 0 && (
                <>
                    ...
                    <S.Members>
                        {groupedMembers.파트장.map((member: TeamMember) => (
                            <Profile key={member.id} {...member} />
                        ))}
                    </S.Members>
                </>
